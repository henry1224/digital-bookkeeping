...
Base branch: `master`. Semua kerja lewat branch, tidak commit langsung ke `master`.

## Aturan Wajib

1. **Sebelum mulai perintah/tugas baru**: pastikan working tree bersih (`git status`). Jika ada perubahan belum tercommit, commit dulu.
...
|---|---|---|
| Dokumen dibuat/diperbaharui | `document/{nama-perbaikan}` | `document/rbac-superset` |
| Code fitur baru/diperbaiki | `feature/{phase}/{nama-menu}` | `feature/mvp/daily-sales` |
...
Aturan token:
- `{phase}` = `mvp`, `fase-2`, atau `fase-3` (ikut roadmap `docs/09-modules/overview.md`).
...
# 1. branch baru dari master terbaru
git checkout master
git pull --ff-only          # jika ada remote
git checkout -b document/nama-perbaikan

# 2. kerja, lalu commit
git add -A
git commit -m "docs: ringkasan perubahan"

# 3. merge ke master (no-ff jaga jejak branch)
git checkout master
git merge --no-ff document/nama-perbaikan -m "merge: docs nama-perbaikan"
